...
## Editable & searchable personal recipes

User-created recipes now have Edit actions on recipe cards and inside the recipe reader. Editing preserves the recipe ID so Firebase-synced notes/favorites continue to point to the same recipe.

The Browse Recipes view now includes a dedicated **My Recipes** filter. It can be combined with ordinary text search and category filters, so users can show only their own recipes and then search within them (for example, My Recipes + Spice Mix + `cajun`).

Previously saved custom recipes remain compatible and continue to receive structured quantity migration for scaling.
