...
The app now includes a Monday–Sunday Weekly Meal Plan built around a two-meal structure:
- a standardized first-meal smoothie around 10–11 a.m.;
- a recipe-linked main meal around 4–5 p.m.

Friday's main-meal selector is restricted to fish and/or shrimp recipes.

Meal plans are stored by Monday week-start date, can be navigated backward/forward by week, and sync through Firebase as `weeklyMealPlans`.

`Add This Week to Shopping List` adds ingredients from each distinct planned recipe once, so repeating the same recipe across multiple days is treated as batch-cooked leftovers rather than multiple separate batches.


## Meal Planner UX update

The weekly planner now supports multiple recipe/components per day rather than one recipe per day.

Improvements:
- Week-at-a-glance Monday–Sunday quick-reference panel
- Multiple recipes/components in the same day's main meal
- Quick-add bar for choosing a day and adding a cookbook recipe
- Per-day Add controls for adding another component
- Existing one-recipe meal plans migrate automatically to the new multi-component format
- Plain-text components are allowed Monday–Thursday and Saturday–Sunday for flexible entries such as leftovers; Friday remains cookbook-linked fish/shrimp only
- Every change auto-saves immediately to local storage and queues Firebase sync
- Shopping-list generation includes all linked recipes in the plan, de-duplicated by recipe ID for batch-cooking use

Friday logic permits fish/shrimp as the protein plus neutral vegetable/rice side recipes; it excludes poultry, meat, eggs, legumes, dairy, and other non-seafood protein entries. If Friday contains planned components, shopping-list generation checks that at least one linked fish/shrimp recipe is present.


## September 2026 instruction audit

This build includes a fresh culinary/instruction QA pass, explicit food-safety endpoint guidance, major rewrites of unreliable wings/gizzard/soup instructions, and a more readable active-section highlight. See `CULINARY_AUDIT_REPORT.txt` for the audit summary.


## Meal planner interaction repair
Replaced datalist/exact-name meal entry with an explicit searchable recipe picker. Day-level and Quick Add controls now open the picker; selecting a recipe adds and auto-saves it immediately. Friday uses the same reliable picker restricted to seafood-compatible recipes/sides. Existing saved weekly plans remain compatible.


## Meal-plan editing update

Each existing planned meal component now has an **Edit** action.
- Linked cookbook recipes can be replaced in place through the searchable recipe picker.
- Plain-text meal notes can be edited directly and saved in place.
- On non-Friday days, an edited recipe can also be converted into a plain-text note through the picker.
- Friday continues to enforce the seafood + compatible sides rule.
- Edits auto-save locally and queue Firebase sync, just like additions/removals.


## Shopping workflow redesign

Shopping is now selective and editable:
- Recipe details use **Choose Shopping Ingredients** instead of adding every ingredient automatically.
- A review dialog lets the user check/uncheck individual ingredients and edit quantities/wording before adding.
- Weekly Meal Plan uses **Review Week Ingredients**, with the same selective review flow across linked planned recipes.
- Shopping List has a manual **Add item** field.
- Every shopping-list row is directly editable.
- Individual shopping items can be deleted.
- Purchased items can be removed as a group without clearing the whole list.
- Copy List and Clear All remain available.
- Shopping changes continue to auto-save locally and sync through the existing Firebase personal-data snapshot.

## Friday manual meal entry

Friday now supports **Manual Entry** in addition to cookbook recipe selection. The UI still presents the fish/shrimp Friday rule as a planning reminder, but manual entries such as “salmon + kale salad” are permitted and editable.

## Runtime repair
- Restored shopping helper functions accidentally omitted from the prior build.
- Fixes the startup error that prevented all 423 built-in recipes from initializing.
- Restores Shopping List Add Item and selective ingredient-picker event initialization.
- Firebase realtime snapshots now merge with current device data instead of replacing it directly.
- Custom recipes are merged by recipe ID when sync snapshots are applied.
